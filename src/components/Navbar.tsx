    ...
    <header className="sticky top-0 z-40 w-full bg-[#111111]/95 backdrop-blur-md border-b border-white/10">
      ...
      <div className="bg-[#FFC700] text-black px-4 py-1.5 text-xs font-black uppercase tracking-wider flex items-center justify-between overflow-hidden">
        <div className="flex items-center gap-2 mx-auto truncate text-center">
          <ShieldCheck className="w-3.5 h-3.5 animate-pulse shrink-0 text-black" />
      ...
      {/* Main Navbar */}
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between py-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3.5">
          ...
          disabled
            ...
            className="lg:hidden p-2.5 text-gray-300 rounded-full bg-[#1A1A1A] border border-white/10 opacity-10 pointer-events-none cursor-default"
            title="Search"
          >
          ...
          {/* Minimal Cart Icon Trigger */}
          <button
            onClick={onOpenCart}
            className="relative p-2 rounded-lg bg-[#181818] border border-white/10 text-white hover:bg-white/10 transition-colors cursor-pointer"
            aria-label="Open shopping cart"
          >
            <ShoppingBag className="w-4 h-4" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#FFC700] text-black text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center">
                {cartCount}
              </span>
            )}
          </button>
